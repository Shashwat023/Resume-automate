...
_PATTERNS: list[tuple[re.Pattern, FieldMatch]] = [
    ...
    (re.compile(r"\blast\s*name\b|\bsurname\b|\bfamily\s*name\b"), FieldMatch("full_name", "last_name")),
    ...
    (re.compile(r"\bportfolio\b|\bwebsite\b|\bpersonal\s*site\b"), FieldMatch("portfolio_url")),
    (re.compile(r"\bcurrent\s*(job\s*)?title\b|\bcurrent\s*role\b"), FieldMatch("current_title")),
    ...
    (re.compile(r"\bexpected\s*salary\b|\bdesired\s*salary\b|\bsalary\s*expectation"), FieldMatch("expected_salary")),
    (re.compile(r"\bcurrent\s*salary\b"), FieldMatch("current_salary")),
    (re.compile(r"\bpostal\s*code\b|\bzip\s*code\b|\bzip\b"), FieldMatch("postal_code")),
    ...
    (re.compile(r"\buniversity\b|\bcollege\b|\balma\s*mater\b"), FieldMatch("university")),
