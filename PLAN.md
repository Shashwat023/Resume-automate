...
**G. Frontend wiring — [x] done for D/E/F's needs; per-row control (H) still pending**
- [x] `QueueControls.tsx` — replaced the "not yet supported" toasts with real calls to `pauseApply`/`resumeApply`/`cancelApply` (new `api/queue.ts` methods), targeting the queue's current application via `queueState.currentJobId`.
- [x] **Real bug found and fixed along the way**: `mapHistoryToQueueState` (`api/queue.ts`) only ever set `currentJobId` from an item with status `running` — a `needs_input` (2FA) item maps to `waiting_for_user`, a *different* status, so `currentJobId` silently went `undefined` the moment a job most needed attention. `CurrentJobCard` (and now the live-view entry point) would have gone blank exactly when the user needed to act. Fixed by including `waiting_for_user` in the "current job" lookup.
- [x] **`LiveView.tsx`** (new) — canvas (`<img>` of JPEG frames) + click/keyboard capture over `WS /ws/apply/{id}/live-view`, coordinates scaled from rendered size to the frame's natural (real viewport) size before forwarding as CDP `Input.dispatch{Mouse,Key}Event` params — the backend's own wire format, confirmed by reading `api/ws.py`/`live_view.py` directly rather than guessing. Opened from a new "Take Control" button on `CurrentJobCard` when status is `waiting_for_user`.
- [x] **`useApplicationLogs` hook** (new) — real `WS /ws/apply/{id}/logs` connection, replacing the hardcoded `logs: []` in `queue.ts`; `LogViewer.tsx` now sources from it instead of the (always-empty) store field.
- [x] **Safety fix in `QueueControls.tsx`**: the generic "Resume Queue" button is now disabled (with an explanatory label) while the current job is `waiting_for_user` — resuming a 2FA-paused application from here, before the user has actually entered the code in Live View, would just re-hit the same prompt and waste a cascade pass. Take Control (Live View) is the only correct entry point for that case.
- [x] `npx tsc -b` and `npx vite build` both clean on every touched file (two pre-existing unrelated errors confirmed via `git diff` to predate this session: `api/resume.ts`'s axios progress-event typing, `app/router.tsx`'s unused import).
- **Not live-verified in a browser**: no dev-server click-through was done this pass (no live 2FA/CAPTCHA scenario was available to safely exercise it end-to-end) — type-checked and built only.
- [ ] Per-row play/pause button on every queue row (not just the current job) — that's Part H, not requested this pass, still pending.
